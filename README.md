...
| 会话隧道 | WebShell 与内网隧道的在线状态、最后检测时间与延迟、一键实测连通性、走隧道的现成扫描命令；**交付纪律提示**：入口不是冰蝎马/哥斯拉马会标红「用户连不上」，有 WebShell 却没有可用 suo5 隧道会提示补隧道 |
| 漏洞战果 | 按严重级排序；**拿到什么权限**列；详情直接渲染可复现的原始 HTTP 请求/响应 |
| 攻击链 | **五阶段攻击链**：信息收集 → 互联网资产权限 → 边界突破 → 内网资产权限 → 靶标权限；每阶段给阶段目标、拿到多少分（含累计）、涉及资产、边界突破阶段的真实隧道；A 竖向 + B 横向（**不含打法要点/工具清单**，执行细节不进链路） |
