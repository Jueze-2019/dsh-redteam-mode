...
* **按作战阶段组织**：攻击链路按五个作战阶段（外网打点 → 撕破口子 → 隧道搭建·内网漫游 → 拿下资产权限 → 靶标系统权限）呈现，每阶段给出目标、手段、工具、ATT&CK 技术与蓝队检测视角。
...
redteam_stages                   五个作战阶段的目标/手段/工具/ATT&CK/蓝队视角 + 本阶段实际得分
...
| 攻击链 | **全链路攻击路径图**：五个作战阶段（外网打点→撕破口子→隧道搭建·内网漫游→拿下资产权限→靶标系统权限），每阶段含阶段目标 / 实际战果 / 手段 / 工具 / ATT&CK / 蓝队检测视角；A 竖向 + B 横向两种布局 |
